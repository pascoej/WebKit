...
def generate_test_values():
    ...
    print("// Based on CTAP 2.2 specification section 6.5.7")
    print()

    # Derive the shared secret using HKDF
    shared_secret = derive_protocol2_shared_secret(example_ecdh_result)
    ...
    for i in range(0, 64, 16):
        ...
        suffix = "," if i < 48 else ""
        print("    " + ", ".join(f"0x{b:02x}" for b in chunk) + suffix)
    print("};")
    print()
    ...
    # Verify our implementation matches the spec requirements
    print("// Verification:")
    print(f"// Shared secret length: {len(shared_secret)} bytes (should be 64)")
    print(f"// HMAC key length: {len(hmac_key)} bytes (should be 32)")
    print(f"// AES key length: {len(aes_key)} bytes (should be 32)")
    print()

    # Generate HMAC example for PIN authentication
    example_pin = "1234"
    pin_hash = hashlib.sha256(example_pin.encode('utf-8')).digest()[:16]

    # HMAC using the HMAC key portion
    pin_auth = hmac.new(hmac_key, pin_hash, hashlib.sha256).digest()

    print("// Example PIN authentication:")
    print(f"// PIN: \"{example_pin}\"")
    print("// PIN hash (first 16 bytes of SHA-256):")
    print("constexpr std::array<uint8_t, 16> examplePinHash {")
    print("    " + ", ".join(f"0x{b:02x}" for b in pin_hash))
    print("};")
    print()

    print("// PIN authentication (HMAC-SHA-256 with HMAC key):")
    print("constexpr std::array<uint8_t, 32> examplePinAuth {")
    print("    " + ", ".join(f"0x{b:02x}" for b in pin_auth[:16]) + ",")
    print("    " + ", ".join(f"0x{b:02x}" for b in pin_auth[16:32]))
    print("};")
